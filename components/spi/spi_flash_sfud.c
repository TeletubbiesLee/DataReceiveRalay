...
#if RT_DEBUG_SFUD
#define DEBUG_TRACE         DEBUG_TRACE("[SFUD] "); DEBUG_TRACE
#else
#define DEBUG_TRACE(...)
#endif /* RT_DEBUG_SFUD */
...
void sfud_log_debug(const char *file, const long line, const char *format, ...) 
{
    ...
    DEBUG_TRACE("[SFUD](%s:%ld) ", file, line);
...
rt_spi_flash_device_t rt_sfud_flash_probe(const char *spi_flash_dev_name, const char *spi_dev_name) 
{
    ...
    if (rtt_dev && sfud_dev && spi_flash_dev_name_bak && spi_dev_name_bak) 
    {
        ...
        /* SFUD flash device initialize */
        {
            ...
            {
                DEBUG_TRACE("ERROR: SPI flash probe failed by SPI device %s.\n", spi_dev_name);
                goto error;
            }
        ...
        if (!strcmp(operator, "probe")) {
            if (argc < 3) {
                ...
                if (sfud_dev->chip.capacity < 1024 * 1024) {
                    DEBUG_TRACE("%d KB %s is current selected device.\n", sfud_dev->chip.capacity / 1024, sfud_dev->name);
                } else {
                    DEBUG_TRACE("%d MB %s is current selected device.\n", sfud_dev->chip.capacity / 1024 / 1024,
                            sfud_dev->name);
                }
            }
        } else {
            if (!sfud_dev) {
                DEBUG_TRACE("No flash device selected. Please run 'sf probe'.\n");
                return;
            }
            if (!rt_strcmp(operator, "read")) {
                if (argc < 4) {
                    ...
                    if (data) {
                        ...
                        if (result == SFUD_SUCCESS) {
                            DEBUG_TRACE("Read the %s flash data success. Start from 0x%08X, size is %ld. The data is:\n",
                                    sfud_dev->name, addr, size);
                            DEBUG_TRACE("Offset (h) 00 01 02 03 04 05 06 07 08 09 0A 0B 0C 0D 0E 0F\n");
                            for (i = 0; i < size; i++) {
                                if (i % 16 == 0) {
                                    DEBUG_TRACE("[%08X] ", addr + i);
                                }
                                DEBUG_TRACE("%02X ", data[i]);
                ...
                if (argc < 4) {
                    ...
                    if (data) {
                        ...
                        if (result == SFUD_SUCCESS) {
                            DEBUG_TRACE("Write the %s flash data success. Start from 0x%08X, size is %ld.\n",
                                    sfud_dev->name, addr, size);
                            DEBUG_TRACE("Write data: ");
                            for (i = 0; i < size; i++) {
                ...
                if (argc < 4) {
                    ...
                    if (result == SFUD_SUCCESS) {
                        DEBUG_TRACE("Erase the %s flash data success. Start from 0x%08X, size is %ld.\n", sfud_dev->name,
                                addr, size);
                    }
                }
                ...
                if (argc < 3) {
                    ...
                    if (result == SFUD_SUCCESS) {
                        DEBUG_TRACE("The %s flash status register current value is 0x%02X.\n", sfud_dev->name, status);
                    }
                } else if (argc == 4) {
                    ...
                    if (result == SFUD_SUCCESS) {
                        DEBUG_TRACE("Write the %s flash status register to 0x%02X success.\n", sfud_dev->name, status);
                    }
                } else {
                ...
                if ((argc > 2 && rt_strcmp(argv[2], "yes")) || argc < 3) {
                    DEBUG_TRACE("DANGER: It will erase full chip! Please run 'sf bench yes'.\n");
                    return;
                }
                ...
                if (write_data && read_data) {
                    ...
                    /* benchmark testing */
                    DEBUG_TRACE("Erasing the %s %ld bytes data, waiting...\n", sfud_dev->name, size);
                    start_time = rt_tick_get();
                    result = sfud_erase(sfud_dev, addr, size);
                    if (result == SFUD_SUCCESS) {
                        time_cast = rt_tick_get() - start_time;
                        DEBUG_TRACE("Erase benchmark success, total time: %d.%03dS.\n", time_cast / RT_TICK_PER_SECOND,
                                time_cast % RT_TICK_PER_SECOND / ((RT_TICK_PER_SECOND * 1 + 999) / 1000));
                    } else {
                        DEBUG_TRACE("Erase benchmark has an error. Error code: %d.\n", result);
                    }
                    /* write test */
                    DEBUG_TRACE("Writing the %s %ld bytes data, waiting...\n", sfud_dev->name, size);
                    start_time = rt_tick_get();
                    ...
                    if (result == SFUD_SUCCESS) {
                        time_cast = rt_tick_get() - start_time;
                        DEBUG_TRACE("Write benchmark success, total time: %d.%03dS.\n", time_cast / RT_TICK_PER_SECOND,
                                time_cast % RT_TICK_PER_SECOND / ((RT_TICK_PER_SECOND * 1 + 999) / 1000));
                    } else {
                        DEBUG_TRACE("Write benchmark has an error. Error code: %d.\n", result);
                    }
                    /* read test */
                    DEBUG_TRACE("Reading the %s %ld bytes data, waiting...\n", sfud_dev->name, size);
                    start_time = rt_tick_get();
                    ...
                    if (result == SFUD_SUCCESS) {
                        time_cast = rt_tick_get() - start_time;
                        DEBUG_TRACE("Read benchmark success, total time: %d.%03dS.\n", time_cast / RT_TICK_PER_SECOND,
                                time_cast % RT_TICK_PER_SECOND / ((RT_TICK_PER_SECOND * 1 + 999) / 1000));
                    } else {
                        DEBUG_TRACE("Read benchmark has an error. Error code: %d.\n", result);
                    }
                } else {
                    DEBUG_TRACE("Low memory!\n");
                }
            ...
            if (result != SFUD_SUCCESS) {
                DEBUG_TRACE("This flash operate has an error. Error code: %d.\n", result);
            }
        }
    }
}
